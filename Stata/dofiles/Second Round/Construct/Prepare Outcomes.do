...
* ---------------------
	...
	* Female businesses, income generating activities and financial access
	gen c1_job_iga_1 = 1 if c1_descprimjob == 1  & c1_job_iga==1
	gen c1_job_iga_2 = 1 if c1_descprimjob == 2  & c1_job_iga==1
	gen c1_job_iga_3 = 1 if c1_descprimjob == 99 & c1_job_iga==1 // Samih: check/reclassify (many seems to be waged emloyed!)
	...
	* Head IGA
	gen c1_headjobiga_1 = 1 if c1_headjobiga == 1 
	gen c1_headjobiga_2 = 1 if c1_headjobiga == 2 | c1_headjobiga == 3 
	gen c1_headjobiga_3 = 1 if c1_headjobiga == 4 | c1_headjobiga == 5
	gen c1_headjobiga_4 = 1 if c1_headjobiga == 99 
	...
	* Woman agency 
	foreach var in 	x9_1 x9_2 x9_3 x9_4 x9_5 x9_6 x9_7 x9_largepurchase x9_dailypurchase x9_wifepersonal ///
	...
	* Conso
	...
	egen conso_food = rowtotal(b3_a_* b3_fooddrink_num)
	gen conso_food_pc = conso_food/b1_introt
	egen conso_nofood = rowtotal(b3_medical_num b3_leisure_num b3_clothes_num b3_publictransport_num ///
						b3_elec_gas_water_num b3_landline_phone_num b3_soap_num b3_otherservice_num b3_medicalexpnum b3_chool_expnum)
	gen conso_nofood_pc = conso_nofood/b1_introt
	foreach var in conso_food conso_food_pc conso_nofood conso_nofood_pc {
			winsor `var', gen(`var'_w) p(0.05)
			}
	egen conso_tot=rowtotal(conso_food conso_nofood)
	egen conso_tot_w=rowtotal(conso_food_w conso_nofood_w)
	egen conso_tot_pc=rowtotal(conso_food_pc conso_nofood_pc)
	egen conso_tot_pc_w=rowtotal(conso_food_pc_w conso_nofood_pc_w)
	
	* Livestock, assets
	replace b2_assetnum19_duck = 0 if b2_assetnum19_duck == -515
	...
	* Psychological well-being
		* consistent signs
		global sign d3_dependsmainly d3_solveproblems d3_have_control d3_trust_worthy d3_achieveanything
		foreach var in $sign  		{
