********************************************************************************
********************************************************************************
*			ADO FILE DEFINING MMAT2TEX UPDTATE COMMAND						   *
********************************************************************************
********************************************************************************
...
* Originally created by Jan Ditzen, this modified version standardized 
* add stars based on a 10, 5 and 1% confidence level. 

* PLEASE DO NOT CIRCULATE OR SHARE WITH ANYONE. 
* ORIGINAL COMMAND AVAILABLE ON SSC
********************************************************************************
********************************************************************************
	...
	function m_significance( coeff , real matrix level ,| real scalar df , real scalar par)
	{
		...
		while (j <= cols(coeff)) {
			...
			while (i <= rows(coeff)) {
				...
				//t= t(df,abs(b/se))	ORIGINAL COMMAND
				//t= t(df,abs(coeff[i,j] :/coeff[i+1,j]))	ORIGINAL COMMAND
				
				t = b :/ se // SAMIH'S ADDED
				pval = 2*ttail(df, abs(t)) // SAMIH'S ADDED
				
				//lev = 1 ORIGINAL COMMAND
				//while (lev <= cols(level)) {   ORIGINAL COMMAND
				//	if (t < level[lev]) {	ORIGINAL COMMAND
				//		output[i,j] = sprintf("%s*",output[i,j])  ORIGINAL COMMAND
				//	}  ORIGINAL COMMAND
				//	lev++ ORIGINAL COMMAND
				// } ORIGINAL COMMAND
				
				if (pval < 0.10) {  // SAMIH'S ADDED
				output[i,j] = sprintf("%s*",output[i,j])  // SAMIH'S ADDED
				}  // SAMIH'S ADDED
				if (pval < 0.05) {  // SAMIH'S ADDED
				output[i,j] = sprintf("%s*",output[i,j])  // SAMIH'S ADDED
				}  // SAMIH'S ADDED
				if (pval < 0.01) {  // SAMIH'S ADDED
				output[i,j] = sprintf("%s*",output[i,j])  // SAMIH'S ADDED
				}  // SAMIH'S ADDED
				i = i + 2
			}
